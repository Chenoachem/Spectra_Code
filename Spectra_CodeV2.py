...
#Make a list of the signal for the spectra accross all the frequencies within the cube.
    ...
    ax1.step(xvals_small/10**6,signal,color='blue', label='Flux Signal')
    ax1.set_title("Orion Nebula "+str(round(xvals[0]/10**6,2))+' Spectral RMS='+str(RMS2) + "Jy"+" Max Signal ="+str(round(max_value,2))+"Jy",fontsize=18)
    ...
    ax2=ax1.twinx()
    ax2.step(xvals_small/10**6,signal/RMS,linewidth=2, color='red', label='SNR Ratio')
    ax2.set_ylabel("Signal to Noise Ratio", fontsize=18)
    ax2.tick_params(labelsize=14)
    #ax1.text(xvals_small[60]/10**6, max_value+4 , , fontsize=18)
    ...
    ax2.set_yticks(np.linspace(ax2ylims[0],
                               ...
                               ax2factor,
                               7))
    ax1.legend(loc='upper left', shadow=True, ncol=2)
    ax2.legend(loc='upper right', shadow=True, ncol=1)	
